          ...
          <div className="flex-1 min-w-0">
            ...
            {value && (
              <div className="mt-2 relative inline-block">
                ...
                <button type="button" onClick={() => onChange('')} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-xs">x</button>
              </div>
            )}
          </div>
        )}
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {value.map((url, i) => (
                  <div key={i} className="relative">
                    ...
                    <button type="button" onClick={() => onChange(value.filter((_, j) => j !== i))} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-xs">x</button>
                  </div>
                ))}
              </div>
            )}
  ...
  return (
    <div className="border border-slate-200 rounded-xl p-3 mb-3 bg-slate-50">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        ...
        <div>
          <label className="block text-[10px] font-semibold text-slate-500 mb-1 uppercase tracking-wide">Price Modifier (BDT)</label>
...
export default function ProductForm({ title, initialData, onSubmit, saving, error, backHref }) {
  ...
  return (
    <form id="product-form" onSubmit={handleSubmit}>
      <div className="flex items-center justify-between bg-white border-b border-slate-100 py-3 mb-6 sticky top-0 z-10 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            type="submit"
            disabled={saving}
            className="flex items-center gap-2 text-white px-5 py-2 rounded-lg text-sm font-semibold transition-colors disabled:opacity-50 bg-blue-600 hover:bg-blue-700"
          >
      ...
      <div className="space-y-5 max-w-4xl">
        <Section title="Basic Information" subtitle="Core product details">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Field label="Original Price (BDT) *" hint="For strikethrough display">
        ...
        <Section title="Variants" subtitle="Colors, sizes, flavors, or any product options">
          <p className="text-xs text-slate-500 mb-3 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
            <strong>Name</strong> = variant label (Color, Size, Flavor) . <strong>Value</strong> = option (Black, XL, Mint) . <strong>+/-BDT</strong> = price adjustment
          </p>
