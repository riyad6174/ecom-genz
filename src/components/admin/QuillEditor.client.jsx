  ...
  return (
    <div>
      <div className="flex flex-wrap gap-1 p-2 bg-slate-50 border-b border-slate-200 items-center">
        ...
        <Btn active={editor.isActive('bulletList')} title="Bullet list" onClick={() => editor.chain().focus().toggleBulletList().run()}>Bullet</Btn>
        <Btn active={editor.isActive('orderedList')} title="Numbered list" onClick={() => editor.chain().focus().toggleOrderedList().run()}>List</Btn>
        <Btn active={false} title="Indent" onClick={() => editor.chain().focus().sinkListItem('listItem').run()}>Indent</Btn>
        <Btn active={false} title="Outdent" onClick={() => editor.chain().focus().liftListItem('listItem').run()}>Outdent</Btn>

        <Sep />

        <Btn active={editor.isActive('link')} title="Link" onClick={addLink}>Link</Btn>
        <Btn active={false} title="Image (URL)" onClick={addImage}>Image</Btn>
        <Btn active={false} title="YouTube embed" onClick={addVideo}>Video</Btn>

        <Sep />

        <Btn active={editor.isActive('blockquote')} title="Blockquote" onClick={() => editor.chain().focus().toggleBlockquote().run()}>Quote</Btn>
        <Btn active={editor.isActive('codeBlock')} title="Code block" onClick={() => editor.chain().focus().toggleCodeBlock().run()}>Code</Btn>
        ...
        <input
          ...
          title="Rows"
        />
        <span className="self-center text-slate-400 text-xs">x</span>
        <input
      ...
      {inTable && (
        <div className="flex flex-wrap gap-1 px-2 py-1.5 bg-blue-50 border-b border-blue-100 items-center">
          ...
          <Btn active={false} danger title="Delete table" onClick={() => editor.chain().focus().deleteTable().run()}>Delete</Btn>
        </div>
      )}
