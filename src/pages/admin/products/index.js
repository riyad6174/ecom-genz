...
export default function AdminProducts() {
  ...
  return (
    <>
      <Head><title>Products | GenZ Admin</title><meta name="robots" content="noindex,nofollow" /></Head>
      <AdminProductLayout title="All Products">
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <Link href="/admin/products/create" className="flex items-center gap-2 text-white px-4 py-2 rounded-lg text-sm font-semibold bg-blue-600 hover:bg-blue-700 transition-colors">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-[11px] uppercase tracking-wider text-slate-500 border-b border-slate-100 bg-slate-50">
                ...
                {products.map((p) => (
                  <tr key={p._id} className="hover:bg-blue-50/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right">
                      <span className="font-bold text-blue-700">{Number(p.price).toLocaleString()}</span>
                      {p.originalPrice > p.price && (
                        <span className="text-xs text-slate-400 line-through ml-1">{Number(p.originalPrice).toLocaleString()}</span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-2">
                        <Link href={`/admin/products/edit/${p._id}`} className="p-1.5 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors">
